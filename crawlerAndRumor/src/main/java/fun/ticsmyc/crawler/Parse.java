...
public class Parse {
    ...
    public static List<Rumor> parseRumorInformation(String rumorInformation) {
        List<Rumor> list = new ArrayList<>();
        JSONArray jsonArray = JSON.parseArray(rumorInformation);
        for (Object jsonObj : jsonArray) {
            Rumor rumorStat = JSON.toJavaObject((JSON) jsonObj, Rumor.class);
            list.add(rumorStat);
            logger.info(rumorStat);
        }
