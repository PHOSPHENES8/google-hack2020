...
import fun.ticsmyc.mapper.RumorMapper;
...
public class InformationDao {
     ...
     * 添加谣言信息
     * */
    public String insertRumor(List<Rumor> rumorList) {
        StringBuilder rumorNews = new StringBuilder();
        RumorMapper rumorMapper = session.getMapper(RumorMapper.class);
        for (Rumor rumor : rumorList) {
            String title = rumor.getTitle();
            Rumor old = rumorMapper.selTitle(title);
            if (old == null ){
                int res = rumorMapper.addRumor(rumor);
                logger.info(res + "");
                session.commit();

                rumorNews.append(rumor.getMainSummary() + " "+(!rumor.isRumorType()) + "<br/>");
            } else {
                System.out.println("rumor is not null!");
            }
        }
        return rumorNews.toString();
    }
     ...
     * 获得目前全部谣言信息
     */
    public List<Rumor> getAllRumor() {
        RumorMapper rumorMapper = session.getMapper(RumorMapper.class);
        List<Rumor> rumorList = rumorMapper.selAll(); //这个函数可能有错误？已知java获得的确实是list
        return rumorList;
    }
